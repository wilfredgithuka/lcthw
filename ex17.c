...
void Database_set (struct Connection *conn, int id, const char *name, const char *email)
{
	...
	addr->set = 1;
	//WARNING BUG, Read how to break it and solve it

	char *res = strncpy(addr->name, name, MAX_DATA);
	//demomstrate the strncy bug
	if (!res)
...
int main(int argc, char *argv[])
{
	...
	if (id >= MAX_ROWS) die("Therez not too many records");
	...
	switch (action) {
		...
		case 's':
			if (argc != 6)
				die("Need an id, email, address to set");
